...
We create a loop and calculate mean pairwise Fst values every 20 generations (up to 100). We plot generations against average pairwise Fst and colour the dots by the significance of the Mantel test.
...
for (i in 2:10)
{
simpops <- run.popgensim(simpops, steps=20, cost.mat,
                         n.offspring=para$n.offspring,n.ind=para$n.ind,
                         para$mig.rate, para$disp.max, para$disp.rate,
                         para$n.allels, para$mut.rate,
                         n.cov=para$n.cov, rec="none")
...
res[i,] <- c((i-1)*20+1, tempfst, mantelp)

cat(paste("Generation:", (i-1)*20+1,"\n"))

 }

plot(res$generation, res$fst, col=(res$mantelp<0.05)+1)
...
Obiously this is not a very suitable map.
