...
C. "Run" the species over the landscape for a number of generations
D. Perform a landscape genetic analysis using a resistance layer.
...
1. Rerun the command above until you "like" the resistance layer. You can change ther parameters (but be aware that p should not be higher than 0.5, otherwise the algorithm does not work reliably)
...
1. Check the pariwise Fsts between populations after the 3 generation have passed. 
Is there now an effect of the landscape on the population structure, yet? 
...
**Example:**
We are interested how many generation is takes until the average Fst values becomes so "different", that the effect of the resistance approach can be detected.
We create a loop and calculate mean pairwise Fst values every 5 generations (up to 100). We plot generations against average pairwise Fst and colour the dots by the significance of the Mantel test.
...
res <- data.frame(generation=1, fst=tempfst, mantelp)
for (i in 2:10)
{
...
res[i,] <- c((i-1)*2+1, tempfst, mantelp)

cat(paste("Generation:", (i-1)*2+1,"\n"))
...
Obiously this is not a very suitable map, basically colors (the amount of red of each pixel is converted into resistance)

To make it a bit more realistic we want to convert the ocean (values around 150) to 
As we can see the ocean has a value around 150 we aim
